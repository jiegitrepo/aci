...
def main():
    ...
    # Login to APIC
    session = acitoolkit.Session('https://192.168.25.100', 'admin', 'admin1234')
    resp = session.login()
    if not resp.ok:
    ...
    acitoolkit.Tenant.subscribe(session, extension)

    template = "{0:70} {1:6}  "
    print(template.format("tenant", "current_health"))
    print(template.format("---------", "----"))
    try:
        while acitoolkit.Tenant.has_events(session, extension):
            if acitoolkit.Tenant.has_events(session, extension):
                health_object = acitoolkit.Tenant.get_fault(session, extension)
